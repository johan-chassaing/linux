...
try:
    ...
    print "CRITICAL | Unable to import supervisor module"
    exit(2)
...
for current_process in c.get_supervisor().getAllProcessInfo():
    nb += 1
    current_state = states[current_process['statename']]
...
if processes_error:
    print "CRITICAL | %d/%d - %s" %(len(processes_error),
                                    nb,
                                    ','.join(processes_error))
    exit(2)
elif processes_unkown:
    print "UNKOWN | %d/%d - %s" %(len(processes_unkown),
                                  nb,
                                  ','.join(processes_unkown))
    exit(3)
elif processes_warning:
    print "WARNING | %d/%d - %s" %(len(processes_warning),
                                   nb,
                                   ','.join(processes_warning))
    exit(1)
else:
    print "OK | %d/%d - %s" %(len(processes_ok),
                              nb,
                              ','.join(processes_ok))
    exit(0)
